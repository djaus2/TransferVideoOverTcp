...
namespace PhotoTimingDjaus.Enums
{
    ...
    public class VideoInfo
    {
        ...
        public void AppendTimesToVideoFilename()
        {
            ...
            try
            {
                string moviesPath = Android.OS.Environment.GetExternalStoragePublicDirectory(Android.OS.Environment.DirectoryMovies).AbsolutePath;
                string sourceFileName = RecordedFilename;
                string destFileName = SelectedFilename;

                string sourcePath = RecordedVideoPath; // Path.Combine(moviesPath, sourceFileName);
                string destinationPath = Path.Combine(moviesPath, destFileName);

                // Instead of directly copying the file, create a new file and write to it
                // This approach can help avoid file locking issues
                ...
                // Notify the Android MediaStore about the new file so it appears in file pickers
                NotifyMediaScanner(destinationPath);
            }
            catch (IOException ex)
            {
                Console.WriteLine($"File access error: {ex.Message}");
                // Set SelectedVideoPath to be the same as RecordedVideoPath if we couldn't create the new file
...
#if ANDROID
        /// <summary>
        /// Notifies Android's MediaScanner about a new file so it appears in file pickers
        /// </summary>
        ...
        private void NotifyMediaScanner(string filePath)
        {
            try
            {
                ...
                Console.WriteLine($"Media scanner notified about: {filePath}");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error notifying media scanner: {ex.Message}");
            }
        }
#endif
